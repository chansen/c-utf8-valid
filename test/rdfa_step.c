...
static bool rdfa_accepts(const char *src, size_t len) {
  return rdfa_run(src, len) == UTF8_RDFA_ACCEPT;
}

#define EXPECT_ACCEPT(src, len) do { \
  char _esc[255*4+1]; \
  TestCount++; \
  if (!rdfa_accepts((src), (len))) { \
