...
stream_check(const char* desc,
  ...
  TestCount++;
  if (got.pending != exp_pending) {
    printf("utf8_valid_stream_check [%s] pending == %zu, expected %zu at line %u\n",
           desc, got.pending, exp_pending, line);
    TestFailed++;
  }
...
#define STREAM(desc, st, src, len, eof, exp_status, exp_consumed, exp_pending, exp_advance) do { \
  utf8_valid_stream_result_t _r = utf8_valid_stream_check(st, src, len, eof); \
  stream_check(desc, exp_status, exp_consumed, exp_pending, exp_advance, _r, __LINE__); \
} while (0)
...
test_streaming(void) {
  ...
  STREAM("4-byte split 3/4", &st, "\x98", 1, false,
         UTF8_VALID_STREAM_PARTIAL, 0, 3, 0);
  ...
  STREAM("split then more 1", &st, "abc\xC3", 4, false,
         UTF8_VALID_STREAM_PARTIAL, 3, 1, 0);
  ...
  STREAM("carried lead then ASCII", &st, "A", 1, false,
         UTF8_VALID_STREAM_ILLFORMED, 0, 0, 0);
  ...
  STREAM("eof after carried lead", &st, "", 0, true,
         UTF8_VALID_STREAM_TRUNCATED, 0, 0, 0);
}
